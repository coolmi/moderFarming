...
export function openFJ4Ywfj(file) {
  let dd = window.dd
  let params = {
    // viewstatus: file.viewstatus, // 是否有pdf预览
...
export function openYwfjorDown(params) {
  let dd = window.dd;
  if (params.viewstatus === 0 || params.viewstatus === '0') {
    params.ispdf = true
  } else {
  ...
  // alert(params.docmentafrescoid);
  let fujianUrl = APILIST.openDingfile4Ywfj_url + '?fileurl=' + params.fileurl + '&filename=' + params.filename;
  api.getDocument(fujianUrl, function (res) {
    dd.ready(function () {
      console.log(res)
      if (res.data && res.data.mediaId !== undefined && res.data.mediaId) {
